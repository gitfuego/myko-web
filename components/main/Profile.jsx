...
export default function({user, hidden, signout, userArtists}) {
  ...
  return (
    <div style={{height: '90%', display: hidden ? 'none' : 'block'}} >
      <div className={styles.profile}>
        ...
        <div className={styles.profilePicContainer} style={{backgroundImage: `url(${user?.profile_pic ?? '/profilePlaceholder.svg'})`}}></div>
      ...
      <div className={styles.myClubsProfile}>
        {userArtists.map(artist => <ClubCardProfile key={'homecard' + artist?.id} artist={artist} />)}
      </div>
    </div>
  );
}